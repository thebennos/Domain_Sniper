<?php function add_job($crontab_list, $user, $start_month, $start_day, $start_hour, $end_month, $end_day, $end_hour, $command, $frequency = 1){
	$f_ptr = fopen($crontab_list, "a+") or die("File could not be opened/created.");
	fwrite($f_ptr, "*/$frequency ".$start_hour."-".$end_hour." ".$start_day."-".$end_day." ".$start_month."-".$end_month." * ".$user." ".$command);
	fclose($f_ptr);
